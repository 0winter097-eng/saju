...
function updateFields(){
 ...
 const mode=$('#time-mode').value;$('#exact-time').hidden=mode!=='exact';$('#range-time').hidden=mode!=='range';
...
$('#load-example').addEventListener('click',()=>{
 form.reset();$('#birth-date').value='1990-05-12';$('#birth-time').value='14:35';$('#sex').value='female';$('#city').value='daegu';example=true;updateFields();calculate();
});
$('#reset').addEventListener('click',()=>{form.reset();$('#birth-date').value='';$('#birth-time').value='';model=null;result=null;dirty=false;example=false;$('#results').innerHTML='';$('#results').hidden=true;$('#empty').hidden=false;$('#status').textContent='입력정보와 계산 결과를 지웠습니다.';$('#form-error').hidden=true;updateFields();});
...
function render(){
 ...
 const subtitle=`양력 ${dateText(m.normalized.solar)} · ${esc(city?.name??'국내')} · ${m.input.timeMode==='unknown'?'시간 미상':m.input.timeMode==='range'?`${esc(m.input.timeStart)}–${esc(m.input.timeEnd)}`:esc(m.input.time)}`;
